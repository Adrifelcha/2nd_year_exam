...
text(-0.22,0.8,expression(paste(mu)), cex=cex.greek, col=color, f=f)
text(-0.07,0.73,"x", cex=cex.greek-0.75, col=color, f=f)
...
text(0.86,-0.23,"y", cex=cex.greek-0.75, col=color, f=f)
...
# #        heights = c(2.5, -0.5, 4), # Heights of the two rows
...
# 
# par(pty="m")            
# par(mfrow=c(2,1),       # A single plot
#     mar = c(5, 1, 0, 1)) # outer margins
# ########################################
# # Top right: Responses
# ########################################
...
# mtext("Decision", 1, f=1, line=2.9, cex = cex.text)
# ########################################
...
# mtext("Response time", 1, f=1, line=2.9, cex = cex.text)

# ##############################################################
# # Figure 3: Discrete examples
# ##############################################################
# # Figure 3A: Color wheel
# ##############################################################
# # Define colors in Color wheel
# ########################################
# # Number of color points to draw
# nPoints = nrow(circle)
# rot = round(nPoints/6,0)+1
# # Define rotations of RGB dimensions
# keepmax <- rep(227,rot)
# keepmin <- rep(28,rot)
...
# octavo = pi/4 #eighth
# bound.list <- list("orange.lowBound" = 0*octavo,
#                    "yellow.lowBound" = octavo,
#                    "green.lowBound" = 2*octavo,
#                    "blue.lowBound" = 3*octavo,
#                    "cyan.lowBound" = 4*octavo,
#                    "purple.lowBound" = 5*octavo,
#                    "pink.lowBound" = 6*octavo,
#                    "red.lowBound" = 7*octavo)
# 
# nT <- length(bound.list)
# # Define alternate list with upper threshold per cat
# up.bound <- bound.list
# up.bound[[1]] = (2*pi)+bound.list[[1]]
# # Identify colors corresponding to each threshold
# color.names <- sub("(^[^-]+)\\..*", "\\1", names(bound.list))
# # Map upper index
# up.idx <- c(2:nT,1)
# # Define length of threshold lines
...
# plot(-10:10,-10:10,type="n", ann = FALSE, axes = FALSE,
#      xlim=c(-pm,pm),ylim=c(-pm,pm))
# # Define von Misses egg
# ########################
# # Vector of all angles for which to compute von Mises density
# full.arc <- seq(from=0,to=2*pi,length.out=(5000-(nT+1)))
# full.arc <- c(full.arc,as.numeric(bound.list),
#               as.numeric(up.bound[!up.bound %in% bound.list]))
# full.arc <- sort(full.arc, decreasing = FALSE)
# # Compute von Mises density
# full.d <- vonMises(a=full.arc,mu=as.numeric(dangle),kappa=Kappa, degrees = FALSE)*1.5
# # Draw the base shade for the whole distribution
...
#     from = j
#     to = up.bound[[idx2]]
#     arc <- seq(from=from,to=to,length.out=500)
# 
#     # Draw shade
...
#     # First part: Truncate density points
#     keep.from = max(which(full.arc %in% from))
#     keep.to = min(which(full.arc %in% to))
...
# }
# text(boundary-0.5,4.2, "Yellow", f=f, cex=cex.text)
# text(boundary+1,1.5, "Orange", f=f, cex=cex.text, srt=-90)
# text(boundary+1,-1.5, "Red", f=f, cex=cex.text, srt=-90)
# text(boundary-1,-4.2, "Magenta", f=f, cex=cex.text)
# text(-2,-4.2, "Purple", f=f, cex=cex.text)
# text(-boundary-1,-1.5, "Blue", f=f, cex=cex.text, srt=90)
# text(-boundary-1,1.5, "Cyan", f=f, cex=cex.text, srt=90)
# text(-boundary+1.5,4.2, "Green", f=f, cex=cex.text)
# for(i in 1:show.trials){
...
# 
# ##############################################################
# # Figure 3B: Spatial location example
# # ##############################################################
# # Defining special variables
# ########################################
# # Define list of angle to draw the lower thresh per cat
# bound.list <- list("Karl.lowBound" = 0.3,
#                    "Amos.lowBound" = (pi/4)-0.02,
#                    "Susan.lowBound" = (3*pi)/4,
#                    "Ada.lowBound" = ((5*pi)/4)-.25,
#                    "Viewer.lowBound" = (6*pi)/4)
# 
# nT <- length(bound.list)
# # Define alternate list with upper threshold per cat
# up.bound <- bound.list
# up.bound[[1]] = (2*pi)+bound.list[[1]]
# # Identify colors corresponding to each threshold
# person.name <- sub("(^[^-]+)\\..*", "\\1", names(bound.list))
# # Map upper index
# up.idx <- c(2:nT,1)
# show.trials <- 10
# 
# color <- paste("gray",round(seq(50,80,length.out=nT),0), sep="")
# ########################################
# # Draw circle
# ########################################
# # Empty canvas
...
#   # Define top [x,y] (at threshold and density)
#   # First part: Truncate density points
#   keep.from = which(full.arc %in% from)
#   keep.to = which(full.arc %in% to)
#   keep <- keep.from:keep.to
#   d.points = full.d[keep]
#   a.points = full.arc[keep]
#   x2_curve = (boundary+d.points)*cos(a.points) # Draw curve
...
#   y.range = c(y1,rev(y2_curve))
#   polygon(x=x.range, y=y.range,
#           xpd=NA, col=color[idx])
# 
# }
# symbols(x=0,y=0,circles=boundary,add=TRUE,inches=FALSE,xpd=NA,
#         fg='black',bg = "white",lwd=20)
# for(i in 1:show.trials){
#   z = seq(1,sum(!is.na(state[,,i])),length.out=55)
#   points(state[z,,i], type = "l", col="#DACF8F", lwd=2)
# }
# for(a in bound.list){
#   run = polarToRect(a,length.threshold)[,1]
#   rise = polarToRect(a,length.threshold)[,2]
#   lines(c(0,run),c(0,rise))
# }
# text(boundary+1,2.5, "Karl", f=f, cex=cex.text, srt=-90)
# text(boundary,-4.2, "Viewer", f=f, cex=cex.text)
# text(-2.5,-4.2, "Ada", f=f, cex=cex.text)
# text(-boundary-1,0.5, "Susan", f=f, cex=cex.text, srt=90)
# text(-boundary+3,4.2, "Amos", f=f, cex=cex.text)
# for(i in 1:show.trials){
#   points(choices[i,1],choices[i,2], type = "p", pch =16, cex=1.5,
#          col="#F4D340")
# }
